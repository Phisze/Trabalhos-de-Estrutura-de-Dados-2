            ...
            for m in {q} if D == C else g:
                ...
                for k in g:
                    lengths_and_k_and_tree.append((len(dij[(m, k)]) + len(SkD[(k, D)][0]) + len(SkD[(k, D)][1]),
                                                   k,
                                                   node_and_path_to_edges(m, dij[(m, k)]).union(SkD[(k, D)][0]).union(
                                                       SkD[(k, D)][1])))
...
# g = create_random_graph(10, 0.10)


g = grafo(direcionado=True)

n, capitais = map(int, (input().split(" ")))
for i in range(0, n):
    for j in range(0, n):
        if i != j:
            g.inserir_aresta(str(i), str(j), i + j * 2)
...
for i in g.get_vertices():
    ...
    l.clear()
terminals = frozenset({'1', '2', '3', '4'})
tree_edges = get_steiner_tree(d, terminals)
print(terminals)
print(tree_edges)
c = 2
render_steiner_solution(d, terminals, tree_edges)
